...
library Errors {
    string constant invalidStardustCasksshipId = "Unknown Stardust Cask ID";
}

/// @title StardustCasksNFT
    ...
    ReentrancyGuard
{
        ...
        ERC1155("")
    {
        ...
        tokenURIs[
            BRONZE_CASK_ID
        ] = "https://gateway.pinata.cloud/ipfs/QmRRnuHVwhoYEHsTxzMcGdrCfthKTS65gnfUqDZkv6kbza"; //Need to update ipfs
        tokenURIs[
            SILVER_CASK_ID
        ] = "https://gateway.pinata.cloud/ipfs/QmRRnuHVwhoYEHsTxzMcGdrCfthKTS65gnfUqDZkv6kbza"; //Need to update ipfs
        tokenURIs[
            GOLD_CASK_ID
        ] = "https://gateway.pinata.cloud/ipfs/QmSrKL6fhPYU6BbYrV97AJm3aM6naGWZK95QntXXZuGQrF"; //Need to update ipfs
        stardustCaskDescriptions[BRONZE_CASK_ID] = "Bronze Stardust Cask";
        stardustCaskDescriptions[SILVER_CASK_ID] = "Silver Stardust Cask";
        stardustCaskDescriptions[GOLD_CASK_ID] = "Golden Stardust Cask";
        contractInitialized = false;
        reservedStardustCask = 0;
    }

    ///@notice Request a new StardustCasks NFT from the owner of the smart contract.
    /// You can request any number of NFTs and pay `silverCaskCost` per NFT
    ///@param _silverNumber Number of StardustCasks NFTs to request

    function purchaseSilverCask(uint256 _silverNumber)
        public
        payable
        nonReentrant
    {
        ...
        require(
            this.balanceOf(this.owner(), SILVER_CASK_ID) - _silverNumber >=
                reservedStardustCask,
            "No available Silver Stardust Casks."
        );
        _safeTransferFrom(
            this.owner(),
            msg.sender,
        ...
        nonReentrant
    {
        ...
        require(
            this.balanceOf(this.owner(), GOLD_CASK_ID) - _goldenNumber >=
                reservedStardustCask,
            "No available Golden Stardust Casks."
        );
        _safeTransferFrom(
            this.owner(),
            msg.sender,
    ...
    ///@param _caskType ID for the StardustCasks NFT (8 for rsilver, 168 for golden)
    ///@param _numberOfCasks The number of StardustCasks NFTs to be minted
    function issueNewCask(
        address _to,
        uint256 _caskType,
        ...
        _mint(_to, _caskType, _numberOfCasks, "");
    }

    function initialStardustCask() external onlyOwner {
        require(contractInitialized == false, "contract initialized already");
        issueNewCask(msg.sender, BRONZE_CASK_ID, 100);
        issueNewCask(msg.sender, SILVER_CASK_ID, 100);
        issueNewCask(msg.sender, GOLD_CASK_ID, 100);
        contractInitialized = true;
    }

    // /// @notice Change the cost for minting a new regular StardustCasks NFT
    // /// Can only be called by the owner of the smart contract.

    /// @notice Return the current cost of minting a new Silver StardustCasks NFT.
    function getSilverCost() external view returns (uint256) {
        return silverCaskCost;
    }
    ...
    function getStardustCasksPrice(uint256 _Price, uint8 _decimals)
        internal
        view
        returns (uint256)
    {
    ...
    //only owner
    function setCitizensAddress(address _newAddress) public onlyOwner {
    ...
    /// @notice Return the number of minted Silver Stardust Cask
    function getTotalBronzeCasks() external view returns (uint256) {
        return mintedBronzeCounter;
    }

    /// @notice Return the number of minted Silver Stardust Cask
    function getTotalSilverCasks() external view returns (uint256) {
        return mintedSilverCounter;
    }

    /// @notice Return the current maximum number of  minted Golden Stardust Cask
    function getTotalGoldenCasks() external view returns (uint256) {
        return mintedGoldCounter;
    }

    /// @notice Return the Cost of Silver Stardust Cask
    function getSilverCasksCost() external view returns (uint256) {
        return getStardustCasksPrice(getCitizensCost(silverCaskCost), 18);
    }

    /// @notice Return the Cost of Golden Stardust Cask
    function getGoldenCasksCost() external view returns (uint256) {
        return getStardustCasksPrice(getCitizensCost(goldenCaskCost), 18);
    }

    /// @notice Withdraw the funds locked in the smart contract,
    /// originating from the minting of new regular StardustCasks NFTs.
    ...
    function reserveStardustCasks(uint256 _numberOfStardustCasks)
        external
        onlyOwner
    {
        reservedStardustCask = _numberOfStardustCasks;
    }

    function howManyreservedStardustCask() external view returns (uint256) {
        return reservedStardustCask;
    }
    ...
    /// @notice Airdrop StardustCasks NFTs to users. The StardustCasks NFTs must first be minted to the owner address.
    function awardStardustCasks(
        address[] calldata _awardees,
        uint256[] calldata _numberOfStardustCasks,
        uint256 _StardustCaskType
    ) external onlyOwner {
        require(
            _awardees.length == _numberOfStardustCasks.length,
            "Array length not equal"
        );
        address Feliz = this.owner();
        for (uint256 i = 0; i < _awardees.length; i++) {
            safeTransferFrom(
                Feliz,
                _awardees[i],
                _StardustCaskType,
                _numberOfStardustCasks[i],
                ""
            );
            if (_StardustCaskType == 68) {
                mintedSilverCounter = mintedSilverCounter.add(
                    _numberOfStardustCasks[i]
                );
            } else if (_StardustCaskType == 168) {
                mintedGoldCounter = mintedGoldCounter.add(
                    _numberOfStardustCasks[i]
                );
            } else if (_StardustCaskType == 8) {
                mintedBronzeCounter = mintedBronzeCounter.add(
                    _numberOfStardustCasks[i]
                );
            }
    ...
    function uri(uint256 _StardustCasksNFTId)
        public
        view
        override
        ...
        string memory json = Base64.encode(
            bytes(
                string(
                    abi.encodePacked(
                        '{ "name": "',
                        stardustCaskDescriptions[_StardustCasksNFTId],
                        '", ',
                        '"description" : ',
                        '"A Stardust Casks from Feliz planet can change to Real Estate and get more benefit in Feliz project.",',
                        '"image": "',
                        tokenURIs[_StardustCasksNFTId],
                        '"'
                        "}"
                    )
                )
    ...
    /// @notice Change the URI of StardustCasks NFTs
    /// @param _tokenURIs Array of new token URIs
    /// @param _StardustCasksNFTIds Array of StardustCasks NFT Ids (168 OR 8) for the respective URIs
    function changeURIs(
        string[] calldata _tokenURIs,
        uint256[] calldata _StardustCasksNFTIds
    ) external onlyOwner {
